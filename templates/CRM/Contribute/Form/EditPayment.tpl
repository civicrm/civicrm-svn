...
{elseif $context NEQ 'standalone'}
    <h3>{if $action eq 4 }{ts}View Payment{/ts}{elseif $action eq 8}{ts}Delete Payment{/ts}{else}{ts}Edit Payment{/ts}{/if}</h3> 
{/if}
...
{/if}
   ...
   {else}
	...
	{if $lineItem and !$defaultContribution}
	    <tr>
        ...
        <tr  class="crm-contribution-form-block-total_amount">
    	    ...
    	    <td {$valueStyle}>
        	    ...
        	    {if $hasPriceSets}		    
        	        <span id='totalAmountORPriceSet'> {ts}OR{/ts}</span>f
...
{/if}	
     ...
     {/if}
        ...
        <tr class="crm-contribution-form-block-source"><td class="label">{$form.source.label}</td><td{$valueStyle}>{$form.source.html|crmReplace:class:huge} </td></tr>
  ...
  <tr class="crm-contribution-form-block-receive_date">
        ...
        <tr id="softCreditID" class="crm-contribution-form-block-soft_credit_to"><td class="label">{$form.soft_credit_to.label}</td>
            <td {$valueStyle}>
        	    ...
        	    {if $siteHasPCPs}
        	        <div id="showPCPLink">{ts}<a href='#' onclick='showPCP(); return false;'>credit this contribution to a personal campaign page</a>{/ts}</div>
        	    {/if}
            </td>
        </tr>
           </table>
