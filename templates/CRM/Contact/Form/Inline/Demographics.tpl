...
function showDeceasedDate( ) {
  ...
  cj('#Demographics').ajaxForm( options );
  ...
  function showRequest(formData, jqForm, options) { 
      ...
      queryString = queryString + '&class_name=CRM_Contact_Form_Inline_Demographics&snippet=5&cid=' + {/literal}"{$contactId}"{literal};
      ...
      if ( status ) {
        ...
        var queryString = 'class_name=CRM_Contact_Page_Inline_Demographics&type=page&cid=' + {/literal}"{$contactId}"{literal};
        var response = cj.ajax({
           type: "POST",
           url: postUrl,
           async: false,
