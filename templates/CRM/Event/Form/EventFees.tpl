    ...
    {if $priceSet}
        ...
        <td class="view-value"><table class="form-layout-compressed">
         {if $priceSet.help_pre AND $action eq 1}
            <tr class="crm-participant-form-block-price_set_help_pre"><td colspan=2><span class="description">{$priceSet.help_pre}</span></td></tr>
         {/if}
        {foreach from=$priceSet.fields item=element key=field_id}
         {if ($element.html_type eq 'CheckBox' || $element.html_type == 'Radio') && $element.options_per_line}
            {assign var="element_name" value=price_$field_id}
            {assign var="count" value="1"}
            <tr class="crm-participant-form-block-price_set_elements"><td class="label">{$form.$element_name.label}</td>
                <td class="view-value">
                <table class="form-layout-compressed">
                <tr class="crm-participant-form-block-price_set_element">	
            {foreach name=outer key=key item=item from=$form.$element_name}
                ...
                {if $element.help_post AND $action eq 1}
                    <tr class="crm-participant-form-block-price_set_element_help_post"><td></td><td><span class="description">{$element.help_post}</span></td></tr>
                {/if}
                </table>
              </td>
            </tr>
          {else}	
            ...
            <tr class="crm-participant-form-block-price_set_element_help_post"><td class="label"> {$form.$element_name.label}</td>
       ...
       {/foreach}
         {if $priceSet.help_post AND $action eq 1}
            <tr class="crm-participant-form-block-price_set_help_post"><td colspan=2><span class="description">{$priceSet.help_post}</span></td></tr>
         {/if}
      </table>
    </td>
</tr>
<tr class="crm-participant-form-block-price_set_calculate"><td></td>
    <td align="left">
    ...
    {else} {* NOT Price Set *}
     ...
     <td class ='html-adjust' colspan=2>
        ...
        {if $discount}
            <tr class="crm-participant-form-block-discount"><td class="label">&nbsp;&nbsp;{ts}Discount Set{/ts}</td><td class="view-value">{$discount}</td></tr>
        {elseif $form.discount_id.label}
            <tr class="crm-participant-form-block-discount_id"><td class="label">&nbsp;&nbsp;{$form.discount_id.label}</td><td>{$form.discount_id.html}</td></tr>
        {/if}
        {if $action EQ 2}
            <tr class="crm-participant-form-block-fee_level"><td class="label">&nbsp;&nbsp;{ts}Event Level{/ts}</td><td class="view-value"><span class="bold">{$fee_level}&nbsp;{if $fee_amount}- {$fee_amount|crmMoney:$fee_currency}{/if}</span></td></tr>
        {else}
            <tr class="crm-participant-form-block-fee_amount"><td class="label">&nbsp;&nbsp;{$form.amount.label}</td><td>{$form.amount.html}
        {/if}
        {if $action EQ 1}
    ...
    {if $accessContribution and ! $participantMode and ($action neq 2 or !$rows.0.contribution_id or $onlinePendingContributionId) }
        <tr class="crm-participant-form-block-record_contribution">
        ...
        <tr id="payment_information" class="crm-participant-form-block-payment_information">
           <td class ='html-adjust' colspan=2>
           <fieldset><legend>{ts}Payment Information{/ts}</legend>
             <table id="recordContribution" class="form-layout" style="width:auto;">
                <tr class="crm-participant-form-block-contribution_type_id">
                ...
                <tr class="crm-participant-form-block-total_amount"><td class="label">{$form.total_amount.label}</td><td>{$form.total_amount.html|crmMoney:$currency}<br/><span class="description">{ts}Actual payment amount for this registration.{/ts}</span></td></tr>
                <tr>
                    <td class="label" >{$form.receive_date.label}</td>
                    <td>{include file="CRM/common/jcalendar.tpl" elementName=receive_date}</td>
                </tr> 
                <tr class="crm-participant-form-block-payment_instrument_id"><td class="label">{$form.payment_instrument_id.label}</td><td>{$form.payment_instrument_id.html}</td></tr>
                <tr id="checkNumber" class="crm-participant-form-block-check_number"><td class="label">{$form.check_number.label}</td><td>{$form.check_number.html|crmReplace:class:six}</td></tr>
                {if $showTransactionId }	
                    <tr class="crm-participant-form-block-trxn_id"><td class="label">{$form.trxn_id.label}</td><td>{$form.trxn_id.html}</td></tr>	
                {/if}	
                <tr class="crm-participant-form-block-contribution_status_id"><td class="label">{$form.contribution_status_id.label}</td><td>{$form.contribution_status_id.html}</td></tr>      
             </table>
           </fieldset>
           </td>
        </tr>
...
{if ($email OR $batchEmail) and $outBound_option != 2}
    <fieldset id="send_confirmation_receipt"><legend>{if $paid}{ts}Registration Confirmation and Receipt{/ts}{else}{ts}Registration Confirmation{/ts}{/if}</legend>  
      <table class="form-layout" style="width:auto;">
		 <tr class="crm-participant-form-block-send_receipt"> 
        ...
        <tr id='notice' class="crm-participant-form-block-receipt_text">
