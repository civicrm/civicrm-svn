...
INSERT INTO {$componentTable} SELECT distinct gc.contact_id FROM civicrm_group_contact gc WHERE gc.group_id = {$exportParams['additional_group']} ON DUPLICATE KEY UPDATE {$componentTable}.contact_id = gc.contact_id";
        ...
        // if its primary export, do not re-compute $fields, since we would have it generated by this point.
        $fields = ( $exportParams['exportOption'] == 1 ) ? $fields : null;
        $query = new CRM_Contact_BAO_Query( null, $returnProperties, $fields, false, false, $queryMode );
