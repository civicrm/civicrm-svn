...
class CRM_Report_Form_Contribute_LoggingDetail extends CRM_Report_Form
{
    private $cid;
    private $db;
    private $log_conn_id;
    ...
    function __construct()
    {
        ...
        if (CRM_Utils_Request::retrieve('revert', 'Boolean', CRM_Core_DAO::$_nullObject)) {
            ...
            CRM_Utils_System::redirect(CRM_Report_Utils_Report::getNextUrl('logging/contribute/summary', 'reset=1', false, true));
        }

        // make sure the report works even without the params
        if (!$this->log_conn_id or !$this->log_date) {
            $dao = new CRM_Core_DAO;
            $dao->query("SELECT log_conn_id, log_date FROM `{$this->db}`.log_civicrm_contribution WHERE log_action = 'Update' ORDER BY log_date DESC LIMIT 1");
            $dao->fetch();
    ...
    function buildQuickForm()
    {
        ...
        if ($this->raw) {
            $this->assign('revertURL', CRM_Report_Utils_Report::getNextUrl('logging/contribute/detail', "$q&revert=1", false, true));
        } else {
            $this->assign('revertURL', CRM_Report_Utils_Report::getNextUrl('logging/contribute/detail', "$q&raw=1", false, true));
        }
    }
