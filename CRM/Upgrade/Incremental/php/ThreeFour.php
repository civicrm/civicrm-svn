...
WHERE  v.option_group_id = g.id 
  ...
  AND  v.name      = %2", array( 1 => array('activity_type', 'String'),
    ...
    function upgrade_3_4_7( $rev ) 
    {
        ...
        $pages = CRM_Core_DAO::executeQuery("SELECT id FROM `civicrm_contribution_page` LEFT JOIN civicrm_uf_join ON entity_table = 'civicrm_contribution_page' AND entity_id = civicrm_contribution_page.id AND module = 'OnBehalf' WHERE is_for_organization = 1 AND civicrm_uf_join.id IS NULL";
        while( $pages->fetch( ) ) {
            $query = "
INSERT INTO civicrm_uf_join
    (is_active, module, entity_table, entity_id, weight, uf_group_id)
VALUES
    (1, 'OnBehalf', 'civicrm_contribution_page', %1, 1, %2)", array( 1 => array( $pages->id, 'Integer'),
