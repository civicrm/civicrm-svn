...
class CRM_Mailing_Form_Component extends CRM_Core_Form {
  ...
  function dataRule($params, $files, $options) {
    ...
    foreach (array('text', 'html') as $type) {
      ...
      if (!empty($dataErrors)) {
        $errors['body_' . $type] = ts('The following errors were detected in %1 message:', array(1 => $type)) . '<ul>' . implode('', $dataErrors) . '</ul><br /><a href="' . CRM_Utils_System::docURL2('CiviMail Action Tokens', TRUE) . '">' . ts('More information on tokens...') . '</a>';
      }
    }
