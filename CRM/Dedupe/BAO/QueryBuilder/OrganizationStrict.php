...
class CRM_Dedupe_BAO_QueryBuilder_OrganizationStrict extends CRM_Dedupe_BAO_QueryBuilder {
    ...
    static function record($rg) {
        ...
        $params = array(
              1 => array(CRM_Utils_Array::value('organization_name',$civicrm_contact,''), 'String'),
              2 => array(CRM_Utils_Array::value('email',$civicrm_email,''), 'String')
          );
        ...
        return array(
            "civicrm_contact.{$rg->name}.{$rg->threshold}" => CRM_Core_DAO::composeQuery("
                ...
                WHERE contact_type = 'Organization'
                  AND (organization_name = %1 OR email = %2)", $params, true)
        );
    }
    ...
    static function internal($rg) {
        $query = "
            ...
            WHERE contact1.contact_type = 'Organization'
              AND (email1.email=email2.email OR contact1.organization_name=contact2.organization_name)
