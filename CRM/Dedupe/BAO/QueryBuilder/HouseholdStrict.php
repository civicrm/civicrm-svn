...
class CRM_Dedupe_BAO_QueryBuilder_HouseholdStrict extends CRM_Dedupe_BAO_QueryBuilder {
    ...
    static function record($rg) {
        ...
        $params = array(
              1 => array(CRM_Utils_Array::value('household_name',$civicrm_contact,''), 'String'),
        ...
        return array(
            "civicrm_contact.{$rg->name}.{$rg->threshold}" => CRM_Core_DAO::composeQuery("
                ...
                WHERE contact_type = 'Household'
                  AND (household_name = %1 OR email = %2)", $params, true)
        );
    }
