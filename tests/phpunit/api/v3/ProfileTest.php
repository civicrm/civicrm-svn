...
class api_v3_ProfileTest extends CiviUnitTestCase
{
    ...
    {        
        ...
        $getParams    = array( 'profile_id' => 25,
                               'contact_id' => $contactId,
                               'version'    => 3);
        $profileDetails = civicrm_api3_profile_get( $getParams );

        $this->assertEquals( 0, $profileDetails['is_error'], "In line " . __LINE__
                             . " error message: " . CRM_Utils_Array::value('error_message', $profileDetails) );
        
        foreach( $updateParams as $profileField => $value ) {
            $this->assertEquals( $value, CRM_Utils_Array::value($profileField, $profileDetails['values']), "In line " . __LINE__
                             . " error message: " . "missing/mismatching value for {$profileField}" ); 
        }
    ...
    {        
        ...
        foreach ( $expected['contact'] as $field => $value ) {
            $this->assertEquals( $value, CRM_Utils_Array::value($field, $result['values']), "In line " . __LINE__
        ...
        foreach( array('email', 'phone', 'address' ) as $fieldType ) {
            ...
            foreach( $expected[$fieldType] as $field => $value ) {
                $this->assertEquals( $value, CRM_Utils_Array::value($field, $typeValues), "In line " . __LINE__
    ...
    function _createIndividualContact( ) {
        ...
        $this->assertEquals( 0, $contact['values'][$contactId]['api.phone.create']['is_error'], "In line " . __LINE__
                             . " error message: " . CRM_Utils_Array::value('error_message', $contact['values'][$contactId]['api.phone.create']) );
