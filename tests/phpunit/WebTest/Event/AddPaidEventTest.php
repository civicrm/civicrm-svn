...
//      $eventInfoStrings = array( $eventTitle, $eventDescription, $streetAddress );
  ...
  function testAddPaidEventWithTemplate()
  {
      ...
      $eventTitle = 'My Conference - '.substr(sha1(rand()), 0, 7);
      $eventDescription = "Here is a description for this conference.";
      $this->_testAddEventInfoFromTemplate( $eventTitle, $eventDescription );
  ...
  function _testAddEventInfo( $eventTitle, $eventDescription ) {
      ...
      $this->type("description", $eventDescription);
  ...
  function _testAddEventInfoFromTemplate( $eventTitle, $eventDescription ) {
      // As mentioned before, waitForPageToLoad is not always reliable. Below, we're waiting for the submit
      // button at the end of this page to show up, to make sure it's fully loaded.
      $this->waitForElementPresent("_qf_EventInfo_upload-bottom");
...
/*
      // Click "enable online reg" if not already checked - can't get this to work :-(
      $this->storeChecked("is_online_registration", "isEnabled");
...
*/
  ...
  function _testVerifyRegisterPage( $registerStrings ){
      ...
      $this->_checkStrings( $registerStrings );
  }
}
