...
class CRM_GenCode_Main {
    ...
    function __construct($CoreDAOCodePath, $sqlCodePath, $phpCodePath, $tplCodePath) {
        ...
        $this->beautifier = new PHP_Beautifier(); // create a instance
        $this->beautifier->addFilter('ArrayNested');
        $this->beautifier->addFilter('Pear'); // add one or more filters
        $this->beautifier->addFilter('NewLines', array( 'after' => 'class, public, require, comment' ) ); // add one or more filters
...
";
    ...
    function generateDAOs($tables) {
        ...
        foreach ( array_keys( $tables ) as $name ) {
            ...
            $this->beautifier->process(); // required
    ...
    function getTable( $tableXML, &$database, &$tables ) {
        ...
        require_once 'CRM/Core/Config.php';
        CRM_Core_Config::singleton(false); // some kind of refresh?
