...
VALUES 
    ( @domainID, 'Hourly' , NULL, '{ts escape="sql" skip="true"}Mailings scheduler{/ts}',          '{ts escape="sql" skip="true"}Sends out scheduled CiviMail mailings{/ts}',                                                       'civicrm_api3', 'job', 'process_mailing',         'user=USERNAME\r\npassword=PASSWORD\r\nkey=SITE_KEY', 0),
    ...
    ( @domainID, 'Hourly' , NULL, '{ts escape="sql" skip="true"}Activity processor{/ts}',          '{ts escape="sql" skip="true"}nserts activity for a contact or a case by retrieving inbound emails from a mail directory{/ts}',  'civicrm_api3', 'job', 'fetch_activities',        'user=USERNAME\r\npassword=PASSWORD\r\nkey=SITE_KEY', 0),
    ...
    ( @domainID, 'Daily' ,  NULL, '{ts escape="sql" skip="true"}Address geocoder{/ts}',            '{ts escape="sql" skip="true"}Goes through addresses and geocodes them (requires enabled Geocoding provider){/ts}',              'civicrm_api3', 'job', 'geocode',                 'version=3\r\n', 0),
    ( @domainID, 'Daily' ,  NULL, '{ts escape="sql" skip="true"}Greeting updater{/ts}',            '{ts escape="sql" skip="true"}Goes through contact records and updates email and postal greetings, or addressee value{/ts}',     'civicrm_api3', 'job', 'update_greeting',         'version=3\r\n', 0),
    ...
    ( @domainID, 'Always' , NULL, '{ts escape="sql" skip="true"}Participant status processor{/ts}','{ts escape="sql" skip="true"}Updates pending event participant statuses based on time{/ts}',                                    'civicrm_api3', 'job', 'process_participant',     'version=3\r\n', 0),
    ( @domainID, 'Always' , NULL, '{ts escape="sql" skip="true"}Membership status processor{/ts}', '{ts escape="sql" skip="true"}Updates membership statuses and conditionally sends renewal reminders{/ts}',                       'civicrm_api3', 'job', 'process_membership',      'version=3\r\n', 0),
    ( @domainID, 'Always' , NULL, '{ts escape="sql" skip="true"}Membership reminder date processor{/ts}','{ts escape="sql" skip="true"}Sets membership renewal reminder dates for current membership records where reminder date is null{/ts}','civicrm_api3', 'job', 'process_membership_reminder_date',     'version=3\r\n', 0);
